...
ProcessNode = Control.extend({
    ...
    _initDefaultProperties: function(){
		this.x = new ScalarProperty("X position", new NullWidget(new AcceptAllInputFilter, !1), 0),
		this.y = new ScalarProperty("Y position", new NullWidget(new AcceptAllInputFilter, !1), 0),
		this.name = new ScalarProperty("node name", new TextWidget(new AcceptAllInputFilter, !1), ""),
...
StartEvent = ProcessNode.extend({
	initialize: function(){
		ProcessNode.prototype.initialize.call(this, "startevent", "start node", {
			src: "images/start_event.png",
			width: 48,
			height: 48
		}),
		this.name.setValue("start")
	},
	render: function(){
...
EndEvent = ProcessNode.extend({
	initialize: function(){
		ProcessNode.prototype.initialize.call(this, "endevent", "end node", {
			src: "images/end_event.png",
			width: 48,
			height: 48
		}),
		this.name.setValue("end")
	},
	render: function(){
		var paper = window.App._paper, attrs = this._attrs;
		this._el = paper.image("", this.x.getValue(), this.y.getValue(), 0, 0).attr(attrs),
...
TextNode = ProcessNode.extend({
	initialize: function(){
		ProcessNode.prototype.initialize.call(this, "textnode", "text node", {"font-size": 12}),
...
UserTask = ProcessNode.extend({
	initialize: function(){
		var self = this;
		ProcessNode.prototype.initialize.call(this, "usertask", "term node", {
			width: 120,
			height: 50,
			radius: 5,
			fill: "90-#fff-#C0C0C0",
